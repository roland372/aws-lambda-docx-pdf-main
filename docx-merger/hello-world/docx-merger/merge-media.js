...
export const prepareMediaFiles = function (files, media) {
    var count = 1;
        ...
        for (var mfile in medFiles) {
            if (/^word\/media/.test(mfile) && mfile.length > 11) {
                ...
                if (!fileHashLookupTable.get(mfile).has(fileUncompressedSize)) {
                    fileHashLookupTable
                        .get(mfile)
                        .set(fileUncompressedSize, calculateFileHash(zip.file(mfile).asUint8Array()));
                }
...
export const updateMediaRelations = function (zip, count, _media) {
...
export const updateMediaContent = function (zip, count, _media) {
...
export const copyMediaFiles = function (base, _media, _files) {
...
const calculateFileHash = function (uint8array) {
